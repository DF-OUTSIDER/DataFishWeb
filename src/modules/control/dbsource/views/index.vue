<template>
  <ContentWrap title="数据源" :message="t('common.message')">
    <Search :schema="allSchemas.searchSchema" @search="setSearchParams" @reset="setSearchParams" />

    <div class="mb-10px">
      <ElButton v-hasPermi="['dbsource::insert']" type="primary" @click="action(null, 'create')">{{
        t('common.add')
      }}</ElButton>
      <ElButton
        v-hasPermi="['dbsource::delete']"
        :loading="delLoading"
        type="danger"
    ...
    >
      <template #action="{ row }">
        <ElButton v-hasPermi="['dbsource::insert']" type="primary" @click="action(row, 'edit')">
          {{ t('common.edit') }}
        </ElButton>
...
import {
  ...
  getDbSourceListApi,
  deleteDbSourceListApi,
  saveDbSourceApi
} from '@/modules/control/dbsource/api'
import { DbSourceType } from '@/modules/control/dbsource/api/types'

const { t } = i18n.global

const formRef = toRef(formProps, 'formExpose')
// 用于显示详细信息
const detailVo = ref<Nullable<DbSourceType>>(null)

const { register, tableObject, methods } = useTable<DbSourceType>({
  getListApi: getDbSourceListApi,
  delListApi: deleteDbSourceListApi,
  response: {
    list: 'list',
    total: 'total'
  }
...
const editAction = async (rowId: number) => {
  ...
  // 设置数据
  const res = await getDbSourceDetailApi(rowId)
  //await nextTick()
  if (res.data) {
...
// 操作列
const action = (row: DbSourceType | null, type: string) => {
  switch (type) {
    case 'create':
...
const submitForm = async () => {
  ...
  elForm.validate(async (valid) => {
    if (valid) {
      // 提交请求
      try {
        const data = unref(formRef)?.formModel as DbSourceType
        if (actionType.value === 'create') {
          await saveDbSourceApi(data)
